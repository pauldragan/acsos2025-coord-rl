...
# ENGINE = sqlalchemy.create_engine("sqlite:///results_good_6motes_5discretization.sqlite3", echo=True)

def main():
    parser = argparse.ArgumentParser(
        prog='Plot scalability results.',
        description='Plot scalability results for all experiments.')

    parser.add_argument("resultsdir", help="Path to sqlite3 files with results.", type=str)
    args = parser.parse_args()

    fig, ax = plt.subplots(nrows=4, ncols=1, sharex=True, figsize=(3.4, 3.0), tight_layout=True, gridspec_kw={'height_ratios': [1.4, 1.4, 1, 1]})
    rew_ax = ax[0]
    exectime_ax = ax[1]
    msgsize_ax = ax[2]
    memory_ax = ax[3]
    ...
    for motenumber in MOTES:
        ...
        for exp_name in EXP_NAMES:
            ...
            ENGINE = sqlalchemy.create_engine("sqlite:///{}/results_{}motes_good.sqlite3".format(args.resultsdir, motenumber), echo=True)
